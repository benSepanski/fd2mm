...
def gmati_coupling(cl_ctx, queue, V, kappa,
    ...
    rhs_grad_op = fd_bind(function_converter, grad_op,
                          source=(Vdim, inner_bdy_id),
                          target=(Vdim, outer_bdy_id))
    rhs_op = fd_bind(function_converter, op, source=(Vdim, inner_bdy_id),
                     target=(V, outer_bdy_id))

    true_sol_grad = fd.project(true_sol_grad, Vdim,
                                  use_slate_for_inverse=False)
    f_grad_convoluted = rhs_grad_op(queue, sigma=true_sol_grad, k=kappa)
    f_convoluted = rhs_op(queue, sigma=true_sol_grad, k=kappa)
    """
        \langle
            f, v
        \rangle_\Gamma
